...
WorkerAttachments.prototype._change_cb = function(error, change) {
  if (error !== null) {
    console.warn("error while listeneing")
    console.warn(error)
    return;
  }
...
// return true if the attachment
// has a state that needs to be processed
...
// return true if the attachment
// has a folder that needs to be processed
// that is exclude own version folders
WorkerAttachments.prototype._checkFolder = function(doc, name) {
  // toplevel
  if (name.split('/').length === 1) return true;
  // no folder ignored if no config
  if (!this.config) return true;
  // or no versions
  if (typeof this.config.versions !== 'object') return true;

  var folder = name.split('/', 1)[0];

  return !_.any(this.config.versions, function(config, version) {
...
WorkerAttachments.prototype._selectAttachments = function(doc) {
  return _.compact(_.map(doc._attachments, function(attachment, name) {
    if (
        ...
        // ignore own processed files if versions specified
        this._checkFolder(doc, name) &&
