...
WorkerAttachments.prototype._setConfig = function(doc) {
  if (doc._deleted) {
    ...
    this.config = _.extend({}, this.options.defaults, this.config);
  }
}
...
WorkerAttachments.prototype._checkStatus = function(stat, doc, attachment) {
...
WorkerAttachments.prototype._selectAttachments = function(doc) {
  ...
  return _.compact(_.map(doc._attachments, function(attachment, name) {
    if (
        ...
        this._checkStatus(stat, doc, attachment) &&
        // ignore own processed files if folder specified
        (!this.config.folder || name.split('/', 1)[0] !== this.config.folder) &&
